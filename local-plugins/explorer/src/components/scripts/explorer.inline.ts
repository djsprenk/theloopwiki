  ...
  try {
    ...
    for (const explorer of allExplorers) {
      ...
      for (const button of explorerButtons) {
        // Unlike the file-tree list (rebuilt from scratch below on every call),
        // these toggle buttons are part of the persistent header shell and survive
        // across repeated "nav"/"render" events without being recreated. Some
        // components (e.g. encrypted-pages, after a successful decrypt) dispatch
        // a bare "render" event outside of a real SPA navigation, so the cleanup
        // pass that normally strips old listeners before a nav doesn't run here -
        // without this guard, a second listener stacks on top of the first and
        // one tap ends up toggling "collapsed" twice, i.e. doing nothing.
        if (button.dataset.explorerToggleBound === "true") continue;
        ...
        cleanupHandlers.push(() => {
          ...
          delete button.dataset.explorerToggleBound;
        });
      }
