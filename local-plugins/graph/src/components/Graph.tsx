...
export interface GraphOptions {
  ...
  /** Slug prefixes (e.g. "the-loop-academy") to never show the graph for. */
...
export default ((userOpts?: Partial<GraphOptions>) => {
  const disabledPathPrefixes = userOpts?.disabledPathPrefixes ?? [];

  const Graph: QuartzComponent = ({ displayClass, cfg, fileData }: QuartzComponentProps) => {
    const slug = (fileData.slug as string) ?? "";
    if (disabledPathPrefixes.some((prefix) => slug.startsWith(prefix))) {
      return null;
    }
